# Реализация учебного алгоритма шифрования S-DES на C++
Алгоритм реализован "в лоб" без оптимизаций. Может работать медленно.

## Сборка библиотеки

- Статическая библиотека без тестов:
```bash
chmod +x ./build.sh
./build.sh static
```
- Динамическая библиотека без тестов:
